...
void main()
{
    Init_UART();
    Init_ADC();
    Select_Channel(6);          //ADCON select channel 6 for ADC (1K-ohm resistor)
    ...
    while(1)
    {
        ...
        RL = (Volts / (4.096 - Volts)) * 1.0;     // Using Ohms Law in order to find resistance from known Vref and recorded voltage
