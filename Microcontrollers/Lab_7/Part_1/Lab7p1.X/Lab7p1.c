...
#define NS_PED_SW   PORTAbits.RA4       // Location of Switch for North/South PED
#define NS_LT_SW    PORTAbits.RA5       // Location of Switch for North/South Left Turn
...
void NIGHT_MODE()
{
    drawtext(35,10, "N", ST7735_WHITE, ST7735_BLACK, TS_1);
    for (dir=0;dir<4;dir++)             // loop to do 4 directions
    {
        update_color(dir, Color_Red);   // Set all to Red
        update_color(dir, Color_Red);   // 
        update_color(dir, Color_Red);   // 
        update_color(dir, Color_Red);   // 
    }

    dir = NS;
    update_color(dir, Color_Green);
    wait_N_seconds(dir, 10);
    update_color(dir, Color_Yellow);
    wait_N_seconds(dir,3);
    update_color(dir, Color_Red);
...
void DAY_MODE()
{
    drawtext(35,10, "D", ST7735_WHITE, ST7735_BLACK, TS_1);
    for (dir=0;dir<4;dir++)             // loop to do 4 directions
    {
        update_color(dir, Color_Red);   // Set all to Red
        update_color(dir, Color_Red);   // 
    ...
    if(SW_NSPED == 1)
    {
        wait_N_seconds_PED(dir, 11);
    }
    
    wait_N_seconds(dir, 8);
    update_color(dir, Color_Yellow);
    wait_N_seconds(dir, 4);
    update_color(dir, Color_Red);
    
    dir = EWLT;
    update_color(dir, Color_Green);
    wait_N_seconds(dir, 7);
    update_color(dir, Color_Yellow);
    wait_N_seconds(dir, 4);
    update_color(dir, Color_Red);
...
{
    char mask;
    mask = 0x03 << (direction * 2);
    PORTD = PORTD & ~mask;          //Clear bits of new color and direction
    mask = Color << (direction * 2);
    PORTD = PORTD | mask;           //Set new color
}
...
void update_misc()
{
    ...
    Volt = ((STEP * 4.0) + 0 ) / 1000.0;		// Voltage from Photoresistor
    //printf("Photoresistor Voltage = %f\r\n", Volt);
    if(Volt < 1.5) 
    {
        Mode_Txt[0] = 'D';
        SW_MODE = 1;
