...
#define heater_sw       PORTCbits.RC1
...
char contr;
int DS3231_tempC, DS3231_tempF, STEP;
...
void INT0_isr(void)
{
    //ISR for turning the alarm buzzer off
    //printf("interrupt 0 (alarm off)");
    alarm_clear_flag = 1;
    INTCONbits.INT0IF = 0;
...
void INT1_isr(void)
{
    //ISR for turning the alarm buzzer on
    //printf("interrupt 1 (alarm on)");
    alarm_match_flag = 1;
    INTCON3bits.INT1IF = 0;
}

//Get IR code from receiver
void INT2_isr(void)
{
    //printf("\nhello from interrupt 2\n");
    unsigned int timer, bit_count;
...
void main()
{
    ...
    {     
        ...
        if (second != old_second)
        {
            ...
            //printf("heater temp/mode/switch: %d / %d / %d\r", set_heater_temp, heater_mode, heater_sw);
			
			//Heater LED
            if (DS3231_tempF < set_heater_temp) heater_on_led = ON;
            ...
            //printf("ALARM: %d:%d:%d/r", alarm_hour, alarm_minute, alarm_second);
            
            
            //Handle Photoresistor RGB LED
            Select_ADC(0);
            STEP = Get_Full_ADC();
            VOLT = (STEP * 4) / 1000;
            //printf("step: %d, voltage: %f\r", STEP, VOLT);
            if(VOLT > 2.5)
            {
                PORTDbits.RD3 = 1;
                PORTDbits.RD4 = 1;
		...
		//get current IR code from remote/receiver and go into a new mode accordingly
        if (nec_ok == 1)
        {
            //printf("Code:%08lx", nec_code);
...
void Do_Setup_Alarm_Time(void)
{
    ...
    printf("alarm values: %d:%d, %d:%d, %d:%d\r\n", alarm_second, setup_alarm_second, alarm_minute, setup_alarm_minute, alarm_hour, setup_alarm_hour);
}    
...
void DS3231_Write_Alarm_1_Time(char Device)
{
    ...
    I2C_Write(alarm_date);              // Write alarm trigger value to DS3231
    I2C_Stop();                         // Stop I2C protocol
    delay_ms(20);                       // Wait 20 ms   
}    
