...
{
private:
	unsigned char	fileDirectory[4][32] = { 0 };	/* a max of N file directories. Each has 32 bytes.
	...
	unsigned short int	FAT16[256] = { 0 };	//256 clusters only
	unsigned char data[1024] = { 0 };		//256 clusters * 4 bytes/cluster = 1024 bytes.

public:
	
	FileDirectory(); //: to initialize all entries in the fileDirectory and FAT16 to be 0; i.e.safe values.

	bool create(int numberBytes);
